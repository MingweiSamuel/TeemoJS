...
parallel('CDragon API', function() {
  ...
  it('gets champion-summary.json', async function() {
    const data = await api.send('data.championSummary', { version, locale });
    assert.ok(data);
    const teemo = data.find(({ alias }) => 'Teemo' === alias);
    assert.ok(teemo);
  });
  it('gets champions/17.json (Teemo)', async function() {
    const teemo = await api.send('data.championById', [ version, locale, 17 ]);
    assert.ok(teemo);
    assert.equal(teemo.title, "the Swift Scout");
  });
  it('gets champions/-1.json (None)', async function() {
    const data = await api.send('data.championById', { version, locale, id: -1 });
    assert.ok(data);
    assert.equal(data.name, 'None');
  });
  it('gets maps.json', async function() {
    const data = await api.send('data.maps', [ version, 'ja_jp' ]);
    assert.ok(data);
  ...
  it('gets summoner-emotes.json', async function() {
    const data = await api.send('data.summonerEmotes', [ version, 'zh_cn' ]);
    assert.ok(data);
